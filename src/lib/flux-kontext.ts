...
if (process.env.FAL_KEY) {
  ...
  console.log(
    `✅ FAL client configured with key: ${process.env.FAL_KEY.substring(
      0,
      10
    )}...`
  )
  ...
  console.error(
    ...
    Object.keys(process.env).filter((key) => key.includes('FAL'))
  )
}
...
export class FluxKontextService {
  ...
  static async editImagePro(
    ...
    try {
      ...
      console.log(
        `📡 Calling fal.subscribe for endpoint: ${FLUX_ENDPOINTS.KONTEXT_PRO}`
      )
      ...
      const result = await fal.subscribe(FLUX_ENDPOINTS.KONTEXT_PRO, {
        ...
        onQueueUpdate: (update) => {
          ...
          if (update.status === 'IN_PROGRESS') {
            console.log(
              'Generation progress:',
              (update as any).logs?.map((log: any) => log.message).join('\n')
            )
          }
        },
      })
      ...
      console.log(`📋 FAL subscribe result:`, {
        ...
        firstImageUrl:
          result.data?.images?.[0]?.url?.substring(0, 50) + '...' || 'N/A',
      ...
      if (!result.data) {
        ...
        throw new Error(
          'FAL API returned no data - this may indicate a service issue or invalid request'
        )
      }
      ...
      if (!result.data.images) {
        ...
        for (const field of possibleFields) {
          if ((result.data as any)[field]) {
            console.log(
              `🔍 Found potential images in data.${field}:`,
              (result.data as any)[field]
            )
  ...
  static async textToImageMax(
    ...
    try {
      ...
      console.log(
        `📡 Calling fal.subscribe for endpoint: ${FLUX_ENDPOINTS.FLUX_MAX_TEXT_TO_IMAGE}`
      )

      const result = await fal.subscribe(
        FLUX_ENDPOINTS.FLUX_MAX_TEXT_TO_IMAGE,
        {
          input: fluxInput,
          logs: true,
          onQueueUpdate: (update) => {
  ...
  static async textToImagePro(
    ...
    try {
      ...
      console.log(
        `📡 Calling fal.subscribe for endpoint: ${FLUX_ENDPOINTS.FLUX_PRO_TEXT_TO_IMAGE}`
      )

      const result = await fal.subscribe(
        FLUX_ENDPOINTS.FLUX_PRO_TEXT_TO_IMAGE,
        {
          input: fluxInput,
          logs: true,
          onQueueUpdate: (update) => {
   ...
   * 将aspect_ratio转换为image_size格式
   */
  static convertAspectRatioToImageSize(
    aspect_ratio?: string
  ):
    | 'square_hd'
    | 'square'
